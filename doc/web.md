...
[![Github Pages](https://img.shields.io/badge/Github%20Pages-222222?style=for-the-badge\&logo=githubpages\&logoColor=white)](https://sagemik.github.io/sqlite3_simple)

在线演示：[Simple 分词器 示例](https://sagemik.github.io/sqlite3_simple) 。代码请参考 [`example/lib/data/impl_web`](../example/lib/data/impl_web) 。

## 快速开始

### 1. 添加 `sqlite3` v3 、`sqlite3_web` 依赖

在 Web 上使用 Simple 分词器，**需要 [`sqlite3: ^3.3.0`](https://github.com/simolus3/sqlite3.dart/tree/sqlite3-3.3.0/sqlite3) 或以上版本（原因见<a href="#wasm-module-loader-requirement">此处</a>）**，[`sqlite_web`](https://github.com/simolus3/sqlite3.dart/tree/main/sqlite3_web) 理论上不受限制但还是建议使用最新版本：
...
SQLite 是基于文件的嵌入式数据库，但在 Web 环境中直接无法访问文件系统，因此需要 `sqlite3_web` 提供的基于 [Web Worker](https://developer.mozilla.org/zh-CN/docs/Web/API/Web_Workers_API/Using_web_workers)（属于后台线程，支持[文件系统 API](https://developer.mozilla.org/zh-CN/docs/Web/API/File_System_API)）管理数据库的**异步 API** 。

`sqlite3` 提供的同步 API 不满足 Web Worker 的异步通信需求，已有项目增加 Web 支持，可能和涉及大量同步异步代码的调整。一种可以复用同步 API 的方法是通过 [`squadron`](https://github.com/d-markey/squadron) 或其他框架自行管理 Web Worker，例如示例中的 [`impl_web/sqlite3_wasm`](../example/lib/data/impl_web/sqlite3_wasm) 就复用了 `Sqlite3Dao` 。但这类通用框架为了兼容任意 Dart 服务，通常会有更多的初始化工作进而影响首屏加载速度。因此除非项目有相关需求，否则还是建议参考 [`impl_web/sqlite3_web`](../example/lib/data/impl_web/sqlite3_web) 使用 `sqlite3_web` 异步 API。
...
随后到本仓库的 Release 中下载 [内置 Simple 分词器的 `sqlite3.wasm`]() ，置于项目的 `web` 或者资源等可访问目录下，以便后续加载使用。
...
>
> 由于目前 [SQLite 不支持在 Web 上加载运行时扩展](https://sqlite.org/forum/forumpost/9273e6c9159502a1842287f4c20aec964a81c01057bdecf0e8137dd8b5c44dd1) ，本项目基于 [sqlite3_wasm_build](https://github.com/simolus3/sqlite3.dart/tree/main/sqlite3_wasm_build) 编译了内置 Simple 分词器的 `sqlite3.wasm` ，编译代码由 [simple-native 分支](https://github.com/SageMik/sqlite3_simple/tree/simple-native) 维护，具体在 [`src/web`](https://github.com/SageMik/sqlite3_simple/tree/simple-native/src/web) 目录下。
...
由于 Simple 涉及文件读写（`fopen`、`read`、`write` ...）等在 Web 环境中不可用的系统调用，WASI 会将其统一为抽象为 `wasi_snapshot_preview1` 模块的 API（`path_open`、`fd_read`、`fd_write` 等）并要求由宿主侧提供实现。`DefaultSimpleWasmModuleLoader` 通过 [`wasd`](https://github.com/medz/wasd) 实现了这些 API，为 Simple 提供了 Web 环境支撑。

如果默认实现不满足要求，可以继承 `SimpleBridgeCallbacks` 和 `WasmModuleLoader` 自行扩展。
...
上述代码需要通过 `dart compile js` 编译为 JavaScript 才能使用，因此需要注意**不能间接或直接包含以 `package:flutter` 开头的依赖。**
...
其中 [`JiebaDictType`](../lib/src/common/jieba_dict_type.dart) 枚举定义了结巴分词所需的所有字典类型，如 `jiebaDict("jieba.dict.utf8")` 等。

### 5. 创建 Fts5 虚表

参考 [SQLite FTS5 Extension](https://sqlite.org/fts5.html) 的说明，根据项目需求创建 Fts5 虚表，例如 [`impl_web/sqlite3_web/sqlite3_web_impl.dart`](../example/lib/data/impl_web/sqlite3_web/sqlite3_web_impl.dart) ：
